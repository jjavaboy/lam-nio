 ...
 */
package lam.dubbo.common.persistence;
...
public class Page<T> {
	...
	private long count = 0;// 总记录数，设置为“-1”表示不查询总数
	...
	//public Page(HttpServletRequest request, HttpServletResponse response, int defaultPageSize){
		...
		if(this.getPageSize()>30){
			this.setPageSize(Integer.valueOf(Global.getConfig("page.pageSize")));
		}*/
	//}
	...
	public String toString() {
        ...
        if (count > 0) {
        	...
        	sb.append("<div id=\"DataTables_Table_0_length\" class=\"dataTables_length\">");
        	sb.append("<label><select size=\"1\" name=\"DataTables_Table_0_length\" onchange=\""+funcName+"("+pageNo+",this.value,'"+funcParam+"');\" aria-controls=\"DataTables_Table_0\">");
        	sb.append("<option "+(pageSize == 5 ? "selected":"")+" value=\"5\">5</option>");
        	sb.append("<option "+(pageSize == 10 ? "selected":"")+" value=\"10\">10</option>");
        	sb.append("<option "+(pageSize == 20 ? "selected":"")+" value=\"20\">20</option>");
        	sb.append("<option "+(pageSize == 30 ? "selected":"")+" value=\"30\">30</option>");
        	sb.append("</select> 条/页</label>");
        	sb.append("</div>");
        	sb.append("</div>");
    		...
    		if (pageNo == first) {// 如果是首页
    			...
    			sb.append("<li class=\"prev\"><a href=\"javascript:\" onclick=\""+funcName+"("+prev+","+pageSize+",'"+funcParam+"');\">← 上一页</a></li>\n");
    		}
        	...
        	//sb.append("<li class=\"next disabled\"><a href=\"#\">下一页 → </a></li></ul></div>");
    		sb.append("</ul>");
        	sb.append("</div>");
        	sb.append("</div>");
        	sb.append("</div>");
            ...
            sb.append("function "+funcName+"(pageNo,pageSize,funcParam){");
            sb.append(" if(true && document.forms[0]){\n");
            sb.append("     var url = document.forms[0].getAttribute(\"action\");\n");
            sb.append("     if(url.indexOf('?')>-1){\n");
            sb.append("         if(url.indexOf('pageNo')>-1){\n");
            sb.append("             var reg = /pageNo=\\d*/g;\n");
            sb.append("             url = url.replace(reg,'pageNo='+pageNo);\n");
            sb.append("         }else{\n");
            sb.append("             url += '&pageNo='+pageNo;\n");
            sb.append("         }\n");
            sb.append("     }else{url += '?pageNo='+pageNo;}\n");
            
            sb.append("     if(url.indexOf('?')>-1){\n");
            sb.append("         if(url.indexOf('pageSize')>-1){\n");
            sb.append("             var reg = /pageSize=\\d*/g;\n");
            sb.append("             url = url.replace(reg,'pageSize='+pageSize);\n");
            sb.append("         }else{\n");
            sb.append("             url += '&pageSize='+pageSize;\n");
            sb.append("         }\n");
            sb.append("     }else{url += '?pageSize='+pageSize;}\n");
